...
func TestKawaiIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .kawaiignore file that ignores .log files
	require.NoError(t, os.WriteFile(".kawaiignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .kawaiignore file
	kawaiignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".kawaiignore"), []byte(kawaiignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .kawaiignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .kawaiignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .kawaiignore in subdir that ignores normal_nested
	subKawaiignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".kawaiignore"), []byte(subKawaiignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .kawaiignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .kawaiignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .kawaiignore")
